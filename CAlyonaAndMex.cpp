...
int main()
{
    ...
    get(m);
    ll res = BIGLL;
    /* I want to be sure that the smallest
     * subarray no matter where is placed
     * is always filled with a sequence of
     * smallest elements, to be sure about this
     * the only way is to repeat the pattern
     * 0...n-1 where n is the size of the smallest
     * subarray. check on paper */
    fori(1,m) {
	get(l);
	get(r);
	res = min(res, r - l + 1);
